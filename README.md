...
|**진행 날짜** | **회차** | **챕터** | **세션** | **진행률** |                                                                                                                                                                        
|------------|-----|------------------------------|------------------------------|---|
| 25/02/05(수) | 1회차 | 제1장 웹과 네트워크의 기본에 대해 알아보자 | 1.1 웹은 HTTP로 나타낸다<br>1.2 HTTP는 이렇게 태어났고 성장했다<br>1.3 네트워크의 기본은 TCP/IP<br>1.4 HTTP와 관계가 깊은 프로토콜은 IP/TCP/DNS | 8% |
| 25/02/06(목) | 2회차 | 제1장 웹과 네트워크의 기본에 대해 알아보자<br><br>제2장 간단한 프로토콜 HTTP | 1.5 이름 해결을 담당하는 DNS <br>1.6 각각과 HTTP와의 관계<br>1.7 URI와 URL<br><br>2.1 HTTP는 클라이언트와 서버 간에 통신을 한다<br>2.2 리퀘스트와 리스폰스를 교환하여 성립 | 13% |
| 25/02/09(일) | 3회차 | 제2장 간단한 프로토콜 HTTP | 2.3 HTTP는 상태를 유지하지 않는 프로토콜<br>2.4 리퀘스트 URI로 리소스를 식별<br>2.5 서버에 임무를 부여하는 HTTP 메소드<br>2.6 메소드를 사용해서 지시를 내리다 | 17% |  
| 25/02/11(화) | 4회차 | 제2장 간단한 프로토콜 HTTP<br><br>제3장 HTTP 정보는 HTTP 메시지에 있다 | 2.7 지속 연결로 접속량을 절약4<br>2.8 쿠키를 사용한 상태 관리<br><br>3.1 HTTP 메시지<br>3.2 리퀘스트 메시지와 리스폰스 메시지의 구조 | 20% |
| 25/02/12(수) | 5회차 | 제3장 HTTP 정보는 HTTP 메시지에 있다 | 3.3 인코딩으로 전송 효율을 높이다<br>3.4 여러 데이터를 보내는 멀티파트<br>3.5 일부분만 받는 레인지 리퀘스트<br>3.6 최적의 콘텐츠를 돌려주는 콘텐츠 네고시에이션 | 24% |
| 25/02/13(목) | 6회차 | 제4장 결과를 전달하는 HTTP 상태 코드 | 4.1 상태 코드는 서버로부터 리퀘스트 결과를 전달한다<br>4.2 2xx 성공(Success)<br>4.3 3xx 리다이렉트(Redirection)<br>4.4 4xx 클라이언트 에러(Client Error)<br>4.5 5xx 서버 에러(Server Error) | 28% |
| 25/02/16(일) | 7회차 | 제5장 HTTP와 연계하는 웹 서버 | 5.1  1대로 멀티 도메인을 가능하게 하는 가상 호스트<br>5.2 통신을 중계하는 프로그램 : 프록시, 게이트웨이, 터널<br>5.3 리소스를 보관하는 캐시 | 32% |
